...
| **Theme as implemented** | [`apps/web/src/index.css`](apps/web/src/index.css): Tailwind v4 `@theme` exposes `--font-console` (**Sora**) and `--font-telemetry` (**JetBrains Mono**); `body` uses `bg-[#080a0c]` and slate-leaning text. `.deck-telemetry` is the inset “chassis” treatment. Palettes **D / E / F / G** below are **design targets**, not yet global CSS variables everywhere. |
