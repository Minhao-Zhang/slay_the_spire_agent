...
2. An index of available knowledge entries (markdown docs under `data/knowledge/`, retrieved as layer `strategy`, plus learned procedural lessons). Each entry has a stable `id`, `layer` (`strategy` | `procedural` | `episodic`), `tags`, and a short `snippet`.

Your job:
- Pick the most relevant entry IDs for the current decision (typically 2–6 IDs).
- Prefer procedural lessons when they clearly match the situation; use `strategy` markdown entries for act/boss/deck guidance when tags match the current screen and act.
